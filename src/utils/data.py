...
def read_uptodate_data(stocks_list, rootpath = os.path.dirname(os.path.abspath("data.py")) ):
    columnsTitles = ["open","high","low","close","adjusted_close","volume"]
    history = pd.DataFrame(columns=columnsTitles)
    history1 = pd.DataFrame(columns=columnsTitles)
    ...
    for i, stock in enumerate(stocks_list):
        ...
        df = pd.read_csv(rootpath + "/datasets/" + stock + ".csv")
        if df.shape[0] < minlen:
            minlen = df.shape[0]
            #if timestamp is a col name not a index 
            time_list = df["timestamp"].tolist()[::-1]        

    for i, stock in enumerate(stocks_list):
        df = pd.read_csv(rootpath + "/datasets/" + stock + ".csv")  
        df = df.sort_index(level = 'timestamp', ascending =False)
        tmpdf = pd.DataFrame({#"stock": stock,
                              ...
                              "close": df1["close"],
                              "volume": df1["volume"],
                              "adjusted_close": df1["adjusted_close"]})
        history = pd.concat([history, tmpdf], axis = 0, ignore_index = True)
    history = history.reindex(columns = columnsTitles)

    '''
    #get the time_list 
    for i, stock in enumerate(stocks_list):
        #read from quandl.get        
        input_name = "EOD/" + stock

        for _ in range(3):
            try:
                df = quandl.get(input_name)
                break
            except Exception as error:
                print(stock," ", error)

        df.columns = ['open','high','low','close','volume','dividend','split','adjusted_open','adjusted_high','adjusted_low','adjusted_close','adjusted_volume']
        df.index.name = 'timestamp'
        if df.shape[0] < minlen:
            #if timestamp is a col name not a index 
            #if timestamp is an index of an df
            minlen = df.shape[0]
            df = df.sort_index(level = "timestamp", ascending = True)
            df.index = df.index.astype(str)
            time_list = df.index.tolist()

    #get the data and put them into history ndarray 
    for i, stock in enumerate(stocks_list):
        ...
        #if timestamps is a col name  
        #if timestamp is a inde
        history = pd.concat([history, tmpdf], axis = 0, ignore_index = True)
    history = history.reindex(columns = columnsTitles)
    # print(history.values.shape)
    history = history.values.reshape(len(stocks_list),-1,len(columnsTitles))
    return history, time_list


def fetch_data(start_time, end_time, window_length, stocks):
    history, time_list = read_uptodate_data(stocks, rootpath = "utils")
    total_length = len(time_list)
    # print("time list is:{}".format(total_length))
    try:
        print("start_time is:{}".format(start_time))
        print("end_time is:{}".format(end_time))
        if start_time in time_list:
            start_time = total_length - time_list.index(start_time)
        else:
            for time in time_list:
                if start_time < time:
                    start_time = total_length - time_list.index(time)
                    break;
        assert isinstance(start_time, int), "not a valid start date"
    except ValueError as e:
        print('start_time error', e)
    try:
        if end_time in time_list:
            end_time = total_length - time_list.index(end_time) 
        else:
            for i,time in enumerate(time_list):
                if (i != len(time_list) - 1 and end_time > time and end_time < time_list[i+1]) or \
                    i == len(time_list) - 1:
                        end_time = total_length - time_list.index(time)
                        break;
        assert isinstance(end_time, int), "not a valid end date"
    except ValueError as e:
        print('end_time error', e)
    assert end_time <= start_time, "start date must be earlier then end date"
    if end_time == 1:
        print("The start date and end date are from %s to %s" %(time_list[-start_time],\
                                                                        time_list[-1]))
        return history[:,-start_time - window_length:,:],\
                time_list[-start_time:], start_time, end_time - 1
    else:
        print("The start date and end date are from %s to %s" %(time_list[-start_time],\
                                                                        time_list[-end_time]))
        return history[:,-start_time - window_length: -end_time,:], \
...
if __name__ == '__main__':
    stocks_list = ["AAL","AAPL"]
    history = read_uptodate_data(stocks_list)
    print(history.shape)
