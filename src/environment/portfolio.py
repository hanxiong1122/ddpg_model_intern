...
class PortfolioEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.src = DataGenerator(history, abbreviation, 
                                 steps=steps, 
    ...
    def _step(self, action):
        ...
        if self.reward_function == 1: # relative price vector of last observation day (close/open)
            close_price_vector = observation[:, -1, 3]
            open_price_vector = observation[:, -1, 0]
            y1 = close_price_vector / open_price_vector
        elif self.reward_function == 2: # relative price vector of last observation day (close/pre_close)
            close_price_vector = observation[:,-1,4]
            pre_close_price_vector = observation[:,-2,4]
